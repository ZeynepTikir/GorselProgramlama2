...
public class oyunKontrol : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        for(int i = 0; i < engeller.Length; i++)
        {
            ...
            Rigidbody2D rigibodyEngel = engeller[i].AddComponent<Rigidbody2D>();
            rigibodyEngel.linearVelocity = new Vector2(arkaPlanHiz, 0);
            rigibodyEngel.gravityScale = 0;
        }
    ...
    public void OyunBitti()
    {
        ...
        for (int i = 0; i < engeller.Length; i++) 
        {
            Rigidbody2D rigibodyEngel = engeller[i].GetComponent<Rigidbody2D>();
            rigibodyEngel.linearVelocity = new Vector2(0, 0);
            rigibodyEngel.gravityScale = 0;
        }
    }
}
